...
const Signup = () => {
  ...
  useEffect(() => {
    if (isNotUsingEmail && isNotUsingNickname && password === checkPassword && password.length > 0) {
  ...
  const ValidateBtn = ({ validCheckFn }) => {
    ...
    return (
      <div onClick={validCheckFn}>중복확인</div>
    )
  }

  const SubmitBtn = () => {
    return activeSubmit ? <button onClick={signinHandler}>회원가입 하기</button> : <button disabled>회원가입 하기</button>
  }
  ...
  return (
    <div>
      ...
      <div className="password_input">
        <h3>비밀번호 입력</h3>
        <input type="password" placeholder="비밀번호 입력" onChange={e => setValue(e, setPassword)} />
      </div>
      ...
      <div className="password_check_input">
        ...
        <input type="password" placeholder="비밀번호 확인"
          onChange={e => { setValue(e, setCheckPassword) }} />
        {
          password === checkPassword ? <span></span> : <span>비밀번호가 다릅니다!</span>
        }
      </div>
      <SubmitBtn />
    </div>
